...
class ArtisanEconomy(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.leaderboard_message_id = None  # NEW: Track donation point leaderboard
    ...
    async def cleanup_old_embeds(self):
        ...
        # -------------------------
        # DONATION LEADERBOARD (in place)
        # -------------------------
        roster_channel = self.bot.get_channel(ARTISAN_ROSTER_CHANNEL_ID)
        if isinstance(roster_channel, discord.TextChannel):
            if self.leaderboard_message_id:
                try:
                    await self.update_donation_leaderboard()
                    logger.info("✅ Refreshed existing donation leaderboard in-place")
                except Exception as e:
                    logger.warning(f"⚠️ Failed to refresh donation leaderboard, posting new one: {e}")
                    await self.post_donation_leaderboard(roster_channel)
                    logger.info("📌 Posted new donation leaderboard (old one missing or invalid)")
            else:
                await self.post_donation_leaderboard(roster_channel)
                logger.info("📌 Posted donation leaderboard (no previous ID found)")
    ...
    async def post_treasury_embed(self, channel: discord.TextChannel):
        ...
        # Build description with better spacing
        desc_lines = [
            f"**Current Balance:** {self.treasury_balance:,} gold",
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        ]
        ...
        if recent:
            ...
            for txn in reversed(recent):
                ...
                transaction_text.append(f"`{date}` **{sign}{amount:,}g** — {desc}\n         *by {user_mention}*\n")
    ...
    async def post_donation_leaderboard(self, channel: discord.TextChannel):
        """Post the donation point leaderboard embed"""
        embed = self.generate_leaderboard_embed()
        view = DonationLeaderboardView(self)
        
        msg = await channel.send(embed=embed, view=view)
        self.leaderboard_message_id = msg.id
        self.save_data()
        logger.info(f"✅ Posted donation leaderboard (ID: {msg.id})")
    
    async def update_donation_leaderboard(self):
        """Update the donation leaderboard embed"""
        if not self.leaderboard_message_id:
            return
        
        channel = self.bot.get_channel(ARTISAN_ROSTER_CHANNEL_ID)
        if not isinstance(channel, discord.TextChannel):
            return
        ...
        try:
            ...
            logger.error(f"Failed to update donation leaderboard: {e}")
    
    def generate_leaderboard_embed(self) -> discord.Embed:
        """Generate the leaderboard embed with table format"""
        # Get all users with the artisan role
        guild = self.bot.guilds[0]  # Assuming single guild bot
        artisan_role = guild.get_role(ARTISAN_OPTIN_ROLE_ID)
        
        # Build leaderboard data
        leaderboard_data = []
        tracked_users = set()
        
        # First, add all members who currently have the role (even with 0 DP)
        for member in guild.members:
            if artisan_role in member.roles:
                user_id_str = str(member.id)
                tracked_users.add(user_id_str)
                donor_record = self.donations.get(user_id_str, {"total_points": 0, "donation_list": []})
                
                total_dp = donor_record.get("total_points", 0)
                donation_count = len(donor_record.get("donation_list", []))
                
                leaderboard_data.append({
                    "name": member.display_name[:20],  # Truncate long names
                    "user_id": member.id,
                    "total_dp": total_dp,
        ...
        # Second, add members who DON'T have the role anymore but DO have donation data
        for user_id_str, donor_record in self.donations.items():
            if user_id_str not in tracked_users:
                # This person has donation data but no longer has the role
                member = guild.get_member(int(user_id_str))
                if member:
                    display_name = f"{member.display_name[:18]}*"  # * indicates no role
                else:
                    display_name = f"User {user_id_str[:8]}*"
        ...
        # Sort by donation points (descending)
        leaderboard_data.sort(key=lambda x: x["total_dp"], reverse=True)
        
        # Build table
        embed = discord.Embed(
            title="🏆 Donation Point Leaderboard",
            description="Tracking contributions to guild work orders",
            color=discord.Color.gold()
        )
        ...
        if not leaderboard_data:
            embed.add_field(
                name="No Data",
                value="No members with the Artisan role found.",
                inline=False
            )
            return embed
        
        # Create table format (showing top entries in description)
        table_lines = [
            "```",
            "Rank | Name                 | Donations | Points",
            "-----+----------------------+-----------+--------"
        ]
        
        for i, entry in enumerate(leaderboard_data[:100], 1):  # Show top 100 in embed
            rank = f"{i:>4}"
            name = f"{entry['name']:<20}"
            donations = f"{entry['donation_count']:>9}"
            points = f"{entry['total_dp']:>7}"
            
            table_lines.append(f"{rank} | {name} | {donations} | {points}")
        
        table_lines.append("```")
        table_lines.append("*Members with * no longer have the Artisan role*")
        
        embed.description = "\n".join(table_lines)
        
        if len(leaderboard_data) > 100:
            embed.set_footer(text=f"Showing top 100 of {len(leaderboard_data)} members")
        else:
            embed.set_footer(text=f"Total members: {len(leaderboard_data)}")
    ...
    def build_work_order_embed(self, order: dict) -> discord.Embed:
        ...
        if created_by:
            desc_lines.append(f"**Created By:** <@{created_by}>")
        ...
        if materials:
            ...
            for mat_name, mat_data in materials.items():
                ...
                material_lines.append(
                    ...
                    f"      Progress: {donated}/{needed} ({remaining} remaining)\n"
                    f"      Reward: {dp_per_item} DP per item\n"
                )
...
class ArtisanControlView(discord.ui.View):
    ...
    @discord.ui.button(label="Create Workorder", style=discord.ButtonStyle.primary, emoji="📋", custom_id="artisan_control:create_workorder")
    ...
    @discord.ui.button(label="View My Donations", style=discord.ButtonStyle.secondary, emoji="📊", custom_id="artisan_control:view_donations")
    ...
    @discord.ui.button(label="Misc Donation", style=discord.ButtonStyle.secondary, emoji="🎁", custom_id="artisan_control:misc_donation")
...
class TreasuryManagementView(discord.ui.View):
    ...
    @discord.ui.button(label="Deposit Gold", style=discord.ButtonStyle.success, emoji="💰", custom_id="treasury:deposit")
    ...
    @discord.ui.button(label="Withdraw Gold", style=discord.ButtonStyle.danger, emoji="💸", custom_id="treasury:withdraw")
    ...
    @discord.ui.button(label="Transaction History", style=discord.ButtonStyle.secondary, emoji="📜", custom_id="treasury:history")
...
class DonationQuantityModal(discord.ui.Modal, title="Enter Donation Amount"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # ✅ IMPORTANT: Respond to interaction FIRST (within 3 seconds)
        await interaction.response.send_message(
        ...
        # --- NOW send DM to the selected Artisan Manager for confirmation ---
        # (This can take time, but we've already responded to the interaction)
...
class DonationConfirmationView(discord.ui.View):
    ...
    async def confirm_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Defer the response (with error handling for expired interactions)
        try:
            await interaction.response.defer()
        except discord.errors.NotFound:
            # Interaction expired - this can happen if user was slow to click
            logger.warning("Donation confirmation interaction expired")
            return
        ...
        await self.cog.update_donation_leaderboard()
    ...
    async def decline_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            # Interaction expired
            logger.warning("Donation decline interaction expired")
            pass
    ...
    async def edit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            # Interaction expired
            logger.warning("Donation edit interaction expired")
            pass
...
class WorkOrderView(discord.ui.View):
    ...
    def __init__(self, cog: ArtisanEconomy, order_id: str):
        ...
        donate_btn = discord.ui.Button(
            ...
            emoji="📦",
            custom_id=f"workorder:{order_id}:donate"
        )
        donate_btn.callback = self.donate_button
        ...
        details_btn = discord.ui.Button(
            ...
            emoji="ℹ️",
            custom_id=f"workorder:{order_id}:details"
        )
        details_btn.callback = self.details_button
        self.add_item(details_btn)
        
        # Add cancel/complete button based on status
        if is_complete:
            action_btn = discord.ui.Button(
                label="Complete Work Order",
                style=discord.ButtonStyle.success,
                emoji="✅",
                custom_id=f"workorder:{order_id}:complete"
            )
        else:
            action_btn = discord.ui.Button(
                ...
                emoji="❌",
                custom_id=f"workorder:{order_id}:cancel"
            )
        action_btn.callback = self.cancel_complete_button
        self.add_item(action_btn)
    
    async def donate_button(self, interaction: discord.Interaction):
    ...
    async def cancel_complete_button(self, interaction: discord.Interaction):
        """Cancel or Complete this work order based on current state (Manager only)"""
        # Get the button that was clicked from the interaction
        button = [component for component in interaction.message.components[0].children 
                 if component.custom_id and ('cancel' in component.custom_id or 'complete' in component.custom_id)][0]
        ...
        if not has_permission:
            action = "complete" if button.label == "Complete Work Order" else "cancel"
            await interaction.response.send_message(
                f"❌ Only guild managers can {action} work orders.",
                ephemeral=True
            )
            return
        ...
        # Check if this is complete or cancel based on button label
        is_complete_action = (button.label == "Complete Work Order")
        ...
        if is_complete_action:
            ...
            confirm_view = CompleteWorkOrderConfirmView(self.cog, self.order_id)
            contributors = order.get('contributors', {})
            ...
            confirm_view = CancelWorkOrderConfirmView(self.cog, self.order_id)
...
class CompleteWorkOrderConfirmView(discord.ui.View):
    ...
    async def confirm_complete(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Edit the confirmation message to show success and remove buttons
        await interaction.edit_original_response(
            content=f"✅ Work order `{self.order_id}` has been completed and archived!",
            view=None
        )
...
class CancelWorkOrderConfirmView(discord.ui.View):
    ...
    async def confirm_cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Edit the confirmation message to show success and remove buttons
        await interaction.edit_original_response(
            content=f"✅ Work order `{self.order_id}` has been cancelled and removed from the channel.",
            view=None
        )
...
class DonationLeaderboardView(discord.ui.View):
    """View for the donation point leaderboard with management buttons"""
    def __init__(self, cog: ArtisanEconomy):
        super().__init__(timeout=None)
        self.cog = cog
    
    @discord.ui.button(label="View Member", style=discord.ButtonStyle.primary, emoji="👤")
    async def view_member(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Show detailed donation history for a selected member"""
        # Open a search modal instead of dropdown (no 25 option limit)
        modal = MemberSearchModal(self.cog, "view")
        await interaction.response.send_modal(modal)
    ...
    async def remove_member(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Remove a member from the leaderboard (admin only)"""
        # Check for manager role
        if not any(role.id in ARTISAN_MANAGER_ROLE_IDS for role in interaction.user.roles):
            await interaction.response.send_message(
                "❌ You need the Artisan Manager role to use this.",
                ephemeral=True
            )
            return
        
        # Open a search modal instead of dropdown
        modal = MemberSearchModal(self.cog, "remove")
        await interaction.response.send_modal(modal)
    
    @discord.ui.button(label="Edit DP", style=discord.ButtonStyle.secondary, emoji="✏️")
    async def edit_dp(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Edit a member's donation points (admin only)"""
        # Check for manager role
        if not any(role.id in ARTISAN_MANAGER_ROLE_IDS for role in interaction.user.roles):
            await interaction.response.send_message(
                "❌ You need the Artisan Manager role to use this.",
        ...
        # Open a search modal instead of dropdown
        modal = MemberSearchModal(self.cog, "edit")
        await interaction.response.send_modal(modal)


class MemberSearchModal(discord.ui.Modal, title="Search for Member"):
    """Modal for searching members by name - works with unlimited members"""
    search_term = discord.ui.TextInput(
        label="Member Name",
        placeholder="Type part of the member's name...",
        required=True,
        max_length=100
    )
    
    def __init__(self, cog: ArtisanEconomy, action: str):
        super().__init__()
        self.cog = cog
        self.action = action  # "view", "remove", or "edit"
    
    async def on_submit(self, interaction: discord.Interaction):
        """Search for members matching the search term"""
        await interaction.response.defer(ephemeral=True)
        
        search = self.search_term.value.lower().strip()
        
        # Build complete list of all members on leaderboard
        guild = interaction.guild
        artisan_role = guild.get_role(ARTISAN_OPTIN_ROLE_ID)
        ...
        for member in guild.members:
            if artisan_role in member.roles:
                user_id_str = str(member.id)
                tracked_users.add(user_id_str)
                donor_record = self.cog.donations.get(user_id_str, {})
                total_dp = donor_record.get("total_points", 0)
        ...
        # Add members without role but with donation data
        for user_id_str, donor_record in self.cog.donations.items():
            if user_id_str not in tracked_users:
                member = guild.get_member(int(user_id_str))
                total_dp = donor_record.get("total_points", 0)
                
                if member:
                    name = f"{member.display_name} (no role)"
                else:
                    name = f"User {user_id_str[:8]} (left)"
        ...
        if not matches:
            await interaction.followup.send(
                f"❌ No members found matching '**{self.search_term.value}**'.\n"
                f"Try a different search term.",
                ephemeral=True
            )
            return
        
        # If exact match (only 1 result), handle directly
        if len(matches) == 1:
            await self.handle_member(interaction, matches[0]["user_id"], matches[0]["name"])
            return
        
        # Multiple matches - show dropdown (max 25)
        if len(matches) > 25:
            await interaction.followup.send(
                f"⚠️ Found **{len(matches)}** matches for '**{self.search_term.value}**'.\n"
                f"Showing first 25. Please be more specific if your member isn't listed.",
                ephemeral=True
            )
            matches = matches[:25]
        
        # Create dropdown with matches
        options = []
        for m in matches:
            options.append(
                discord.SelectOption(
                    label=m["name"][:100],
                    description=f"DP: {m['total_dp']}",
                    value=m["user_id"]
                )
            )
        
        view = MemberMatchesView(self.cog, self.action, options)
        await interaction.followup.send(
            f"Found **{len(matches)}** match(es) for '**{self.search_term.value}**':",
            view=view,
            ephemeral=True
        )
    
    async def handle_member(self, interaction: discord.Interaction, user_id: str, member_name: str):
        """Handle the selected member based on action type"""
        if self.action == "view":
            # Show member details
            donor_record = self.cog.donations.get(user_id, {})
            total_dp = donor_record.get("total_points", 0)
            donation_list = donor_record.get("donation_list", [])
            
            embed = discord.Embed(
                title=f"📊 Donation History: {member_name}",
                color=discord.Color.blue()
            )
            
            embed.add_field(
                name="Total Donation Points",
                value=f"**{total_dp} DP**",
                inline=True
            )
            ...
            embed.add_field(
                ...
                value=f"**{len(donation_list)}**",
                inline=True
            )
            
            if donation_list:
                recent_donations = donation_list[-10:]
                donation_text = []
                for don in reversed(recent_donations):
                    material = don.get("material", "Unknown")
                    quantity = don.get("quantity", 0)
                    rarity = don.get("rarity", "Common")
                    total_dp_item = don.get("total_dp", 0)
                    date = don.get("date", "")[:10]
                    
                    donation_text.append(
                        f"`{date}` **{quantity}× {material}** ({rarity}) → +{total_dp_item} DP"
                    )
                
                embed.add_field(
                    name=f"📜 Recent Donations (Last {len(recent_donations)})",
                    value="\n".join(donation_text),
                    inline=False
                )
            else:
            ...
            view = ConfirmRemoveMemberView(self.cog, user_id)
            ...
            await interaction.followup.send(
                ...
                f"This will permanently delete all their donation data.\n"
                f"Are you sure?",
                view=view,
                ephemeral=True
            )
            ...
            # Open DP edit modal
            donor_record = self.cog.donations.get(user_id, {})
            current_dp = donor_record.get("total_points", 0)
            
            modal = EditDPModal(self.cog, user_id, member_name, current_dp)
            await interaction.response.send_modal(modal)


class MemberMatchesView(discord.ui.View):
    """View for selecting from multiple search matches"""
    def __init__(self, cog: ArtisanEconomy, action: str, options: list):
        super().__init__(timeout=60)
        self.cog = cog
        self.action = action
    ...
    async def member_selected(self, interaction: discord.Interaction):
        """Handle member selection from matches"""
        selected_user_id = interaction.data["values"][0]
        
        # Get member name
        member = interaction.guild.get_member(int(selected_user_id))
        donor_record = self.cog.donations.get(selected_user_id, {})
        
        if member:
            member_name = member.display_name
        else:
            member_name = f"User {selected_user_id}"
        ...
        if self.action == "view":
            ...
            view = ConfirmRemoveMemberView(self.cog, selected_user_id)
            ...
            current_dp = donor_record.get("total_points", 0)
            modal = EditDPModal(self.cog, selected_user_id, member_name, current_dp)
            await interaction.response.send_modal(modal)


class ConfirmRemoveMemberView(discord.ui.View):
    ...
    @discord.ui.button(label="Yes, Remove", style=discord.ButtonStyle.danger, emoji="✅")
    async def confirm_remove(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Actually remove the member"""
        if self.user_id in self.cog.donations:
            del self.cog.donations[self.user_id]
            self.cog.save_data()
            ...
            member_name = member.display_name if member else f"User {self.user_id}"
            
            await interaction.response.edit_message(
                content=f"✅ Removed **{member_name}** from the donation leaderboard.",
                view=None
            )
        else:
            await interaction.response.edit_message(
                content="❌ Member not found in donation records.",
                view=None
            )
...
class EditDPModal(discord.ui.Modal, title="Edit Donation Points"):
    """Modal for editing a member's DP"""
    new_dp = discord.ui.TextInput(
        label="New Donation Points",
        placeholder="Enter new total DP (or +50 / -50 for relative)",
        required=True,
        max_length=10
    )
    
    def __init__(self, cog: ArtisanEconomy, user_id: str, member_name: str, current_dp: int):
        super().__init__()
        self.cog = cog
        self.user_id = user_id
        self.member_name = member_name
        self.current_dp = current_dp
        self.new_dp.default = str(current_dp)
    
    async def on_submit(self, interaction: discord.Interaction):
        """Process DP edit"""
        value_str = self.new_dp.value.strip()
        
        try:
            # Check if it's a relative change (+50 or -50)
            if value_str.startswith('+') or value_str.startswith('-'):
                change = int(value_str)
                new_total = max(0, self.current_dp + change)
                action = "increased" if change > 0 else "decreased"
            else:
                new_total = int(value_str)
                action = "set to"
            
            # Update donation record
            if self.user_id not in self.cog.donations:
                self.cog.donations[self.user_id] = {"total_points": 0, "donation_list": []}
            
            old_total = self.cog.donations[self.user_id].get("total_points", 0)
            self.cog.donations[self.user_id]["total_points"] = new_total
            ...
            if isinstance(log_channel, discord.TextChannel):
                log_embed = discord.Embed(
                    title="✏️ Donation Points Edited",
                    color=discord.Color.orange()
                )
                log_embed.add_field(name="Member", value=self.member_name, inline=True)
                log_embed.add_field(name="Edited By", value=interaction.user.mention, inline=True)
                log_embed.add_field(name="Old DP", value=str(old_total), inline=True)
                log_embed.add_field(name="New DP", value=str(new_total), inline=True)
                log_embed.add_field(name="Change", value=f"{new_total - old_total:+d}", inline=True)
                log_embed.timestamp = datetime.utcnow()
                await log_channel.send(embed=log_embed)
            
            await interaction.response.send_message(
                f"✅ **{self.member_name}**'s donation points {action} **{new_total} DP**\n"
                f"(Previous: {old_total} DP)",
                ephemeral=True
            )
            
        except ValueError:
            await interaction.response.send_message(
                "❌ Invalid number. Use a number like `500` or a relative change like `+50` or `-50`",
                ephemeral=True
            )
